...
import { fetchFinishedUserOrders } from '../mintme-extension';
...
function parseArgs() {
    ...
    // Define help text
    const helpText = `
Usage: npx ts-node src/tests/fetch_finished_orders.ts [options]
...
Example:
  npx ts-node src/tests/fetch_finished_orders.ts --offset=0 --limit=20
`;
...
/**
 * Example of fetching finished orders with MintMe via CCXT
 */
async function testFetchFinishedOrders() {
    try {
        console.log('=== MintMe Finished Orders Fetch Test ===');
        ...
        console.log('\nFetching finished orders from MintMe with these parameters:');
        console.log(`- Offset: ${offset}`);
        console.log(`- Limit: ${limit}`);
        
        // Send the request
        console.log('\nSubmitting request to MintMe...');
        const result = await fetchFinishedUserOrders(offset, limit);
...
// Execute the test
testFetchFinishedOrders().then(() => {
    console.log('\nMintMe finished orders fetch test completed!');
}).catch(error => {
